...
export default function NewspaperBlurb({ 
  ...
  return (
    <motion.div 
      ...
      className={`border-y-2 border-black py-12 md:py-16 my-8 md:my-16 relative overflow-hidden ${className}`}
    >
      {/* Header Section */}
        ...
        <div className="flex items-start justify-between gap-4 md:gap-6 flex-wrap">
          <div className="flex-1">
            <div className="flex items-center gap-2 md:gap-3 mb-2 md:mb-3">
              {Icon && <Icon className="w-6 h-6 md:w-8 md:h-8 text-black" strokeWidth={2.5} />}
              <div className="h-0.5 md:h-1 bg-black flex-grow"></div>
            </div>
            
            <h3 className="font-['Manrope'] text-2xl md:text-5xl uppercase leading-none mb-3 md:mb-4">
              {title}
            </h3>
            ...
            <div className="flex flex-wrap items-center gap-2 md:gap-4 font-['IBM_Plex_Mono'] text-[10px] md:text-xs uppercase tracking-widest">
              <div className="flex items-center gap-1.5 md:gap-2 bg-black text-white px-2 md:px-3 py-1">
              ...
              <span className="px-2 py-1 border border-black bg-cmyk-cyan text-black font-bold text-[9px] md:text-xs">{tag}</span>
            </div>
          </div>
        </div>
      </div>
